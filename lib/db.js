...
import { sanitizeStacks } from './normalize.js';
...
async function migrateNeon(sql) {
  ...
  try {
    ...
    try {
      await sql`ALTER TABLE jobs ADD COLUMN IF NOT EXISTS is_anonymous BOOLEAN DEFAULT FALSE;`;
      await sql`ALTER TABLE jobs ADD COLUMN IF NOT EXISTS og_image_url TEXT;`;
    } catch (colErr) {
      // Column might already exist or permission restricted
    }
...
export async function createJob({
  ...
  const sanitizedStacks = sanitizeStacks(stacks);
    ...
    try {
      const rows = await db.sql`
        INSERT INTO jobs (
          ...
          is_anonymous, og_image_url, status, views, source, created_at
        ) VALUES (
          ${id}, ${slug}, ${hirerId}, ${title}, ${company}, ${logoUrl}, ${location},
          ${region}, ${workplaceType}, ${jobType}, ${salary}, ${description},
          ${JSON.stringify(sanitizedStacks)}, ${JSON.stringify(customQuestions)},
          ${Boolean(isAnonymous)}, ${ogImageUrl}, 'active', 0, 'Direct Hirer', ${createdAt}
        ) RETURNING *
      `;
      return rows[0];
    } catch (e) {
...
export async function getJobById(id) {
  ...
  if (db.type === 'neon') {
    try {
      ...
      if (rows[0]) {
        ...
        return {
          ...rows[0],
          stacks: sanitizeStacks(typeof rows[0].stacks === 'string' ? JSON.parse(rows[0].stacks) : (rows[0].stacks || [])),
  ...
  if (job) {
    ...
    job.stacks = sanitizeStacks(job.stacks);
    saveLocalStore(store);
  }
  return job || null;
}
...
export async function getJobsByHirerId(hirerId) {
  ...
  if (db.type === 'neon') {
    try {
      ...
      return rows.map(r => ({
        ...r,
        stacks: sanitizeStacks(typeof r.stacks === 'string' ? JSON.parse(r.stacks) : (r.stacks || [])),
  ...
  return store.jobs
    ...
    .map(j => {
      ...
      return { ...j, stacks: sanitizeStacks(j.stacks), applications_count: appsCount };
    });
}
...
export async function getDbJobs({ region = '', category = '' } = {}) {
  ...
  if (!jobs.length) {
    ...
    jobs = (store.jobs || [])
      ...
      .map(j => ({
        ...j,
        stacks: sanitizeStacks(j.stacks),
