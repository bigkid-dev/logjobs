...
export function sanitizeStacks(stacks = []) {
  if (!Array.isArray(stacks)) return [];
  return [...new Set(stacks)]
    .filter(s => typeof s === 'string' && s.trim() && !s.toLowerCase().includes('confidential'));
}

export function extractStacks(text = '') {
  const lower = text.toLowerCase()
  return sanitizeStacks(
    Object.entries(STACK_KEYWORDS)
